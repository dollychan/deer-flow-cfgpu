...
class DynamicContextMiddleware(AgentMiddleware):
    ...
    @staticmethod
    def _ordered_injection(new_messages: list, all_messages: list, target_idx: int) -> dict:
        """Build a state update that places *new_messages* at *target_idx*, preserving order.
        ...
        a second user turn was appended in the same thread), the freshly-minted
        ``{id}__memory`` / ``{id}__user`` messages would land after them instead of
        immediately after the reminder SystemMessage.
        ...
        includes every message that earlier middlewares may have added this turn.  The
        original target message is dropped from the rebuilt list because the reminder
        SystemMessage in *new_messages* reuses its ID and takes its slot.
        ...
        correct_order = all_messages[:target_idx] + list(new_messages) + later_messages
    ...
    def _inject(self, state) -> dict | None:
        ...
        if last_date is None:
            ...
            return self._ordered_injection(result_msgs, messages, first_idx)
        ...
        return self._ordered_injection(result_msgs, messages, last_human_idx)
