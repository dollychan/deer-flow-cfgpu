...
| policy | 准入 | 转存 | 典型 |
|---|---|---|---|
| ``rehost``   | 是 | 临期 url → fetch+upload → object_key | cfgpu generate_*/task_wait/task_status |
...
**解析顺序**：builtin `tool.metadata["materials_capture"]` → cfgpu 内置默认（`cfgpu_` 前缀 →
rehost）→ 默认 off。cfgpu MCP overlay 的 config 驱动版（`materials.capture: {glob:{policy}}`）
待 api-token interceptor 配置层落地后接入；现以 cfgpu_ 前缀硬编码默认兜底（与 §4.3 出口签发
同一前缀约定）。**rehost 的准入信号不靠工具名猜**：cfgpu 媒体结果自声明顶层 ``artifact: true``
（result_structure.json 权威），非媒体结果（list_models/异步 stub/error）无此标志 → rehost
policy 下也抽不到 url → 自然 no-op。故 cfgpu_ 全量给 rehost 安全。
"""
...
def resolve_capture_policy(tool_name: str, tool_metadata: Mapping[str, Any] | None = None) -> CapturePolicy:
    """解析某工具结果的 Capture policy。builtin metadata 优先，其次 cfgpu_ 默认 rehost，否则 off。"""
    if tool_metadata:
        declared = tool_metadata.get("materials_capture")
        if declared in _VALID:
            return declared  # type: ignore[return-value]
    if tool_name.startswith(_CFGPU_PREFIX):
        return "rehost"
    return "off"
