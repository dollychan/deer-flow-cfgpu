...
This file guards three behaviour-preserving deerflow seams and their director overrides:
...
Base assertions pin byte-for-byte upstreamable behaviour; director assertions pin the
...
def test_director_shutdown_destroys_nothing():
    """Director ``_destroy_on_shutdown`` is a no-op: zero autonomous destroy power (I16)."""
    backend = MagicMock()
...
def test_director_idle_cleanup_is_noop():
...
def test_director_evict_oldest_warm_never_destroys():
    """Director never capacity-destroys a warm container (another instance may reclaim it)."""
    backend = MagicMock()
    provider = _provider(_director_cls(), backend=backend)
    provider._warm_pool["wwww"] = (_info("wwww"), 0.0)
...
def test_director_creation_lock_path_on_local_disk(monkeypatch, tmp_path):
    """R3: director relocates the creation lock off virtiofs onto per-host local disk."""
    from deerflow.config import paths as paths_mod
    ...
    provider = _provider(_director_cls())
    lock_path = provider._creation_lock_path("t-1", "abcd1234", user_id="alice")
...
DEPLOY_PROVIDER_PATH = "app.consumer.director_sandbox:DirectorAioProvider"
...
def test_deploy_config_provider_path_resolves_to_sandbox_provider():
    ...
    If `DirectorAioProvider` is ever renamed/moved, this fails in CI instead of
