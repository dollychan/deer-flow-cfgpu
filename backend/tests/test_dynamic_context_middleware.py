...
def test_injects_only_into_first_human_message_not_later_ones():
    """Reminder targets the first HumanMessage; later messages are preserved in order.

    When messages already exist *after* the first-human injection target, the new
    ``{id}__user`` message (a fresh ID) would otherwise be appended at the very end
    by ``add_messages`` — landing after ``Second`` and corrupting the turn order.
    ``_ordered_injection`` rewrites the whole list via ``REMOVE_ALL_MESSAGES`` so the
    reminder→user pair stays at the front and the later messages (``Reply``,
    ``Second``) are carried through unchanged, in their original order.
    """
    mw = _make_middleware()
    state = {
        "messages": [
    ...
    assert result is not None
    msgs = result["messages"]
    # Later messages exist → full reorder via REMOVE_ALL_MESSAGES, preserving order.
    assert msgs[0].id == REMOVE_ALL_MESSAGES  # discards the old list
    rebuilt = msgs[1:]
    assert [m.id for m in rebuilt] == ["msg-1", "msg-1__user", rebuilt[2].id, "msg-2"]
    # Reminder (SystemMessage) takes the first message's ID.
    assert isinstance(rebuilt[0], SystemMessage)
    assert rebuilt[0].additional_kwargs.get(_DYNAMIC_CONTEXT_REMINDER_KEY) is True
    assert _SYSTEM_REMINDER_TAG in rebuilt[0].content
    # Original first-user content moves to the derived ``__user`` ID.
    assert rebuilt[1].id == "msg-1__user"
    assert rebuilt[1].content == "First"
    # "Second" (msg-2) is carried through unchanged, at the end (correct order).
    assert rebuilt[-1].id == "msg-2"
    assert rebuilt[-1].content == "Second"
