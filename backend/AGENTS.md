...
4. **ThreadDataMiddleware** - Creates per-thread directories keyed by `thread_id` alone (`backend/.deer-flow/threads/{thread_id}/user-data/{workspace,uploads,outputs}`). Thread data is isolated by `thread_id`, never per-user, so multiple users sharing a thread share its materials (cfgpu-docs/thread-tenancy.md; `user_id` still scopes memory/agents/billing/tracing, just not thread-data disk). Path helpers accept a `user_id` kwarg for call-site signature compatibility but ignore it for thread-data disk layout
...
**Thread-scoped file storage**: inbound files, uploads, and output artifacts are staged in the thread bucket so they land where the agent run reads/writes (`threads/{thread_id}/user-data/{uploads,outputs}` — thread-only tenancy, cfgpu-docs/thread-tenancy.md). `ChannelManager._handle_chat` still resolves a storage owner via `_channel_storage_user_id(msg)` and threads it as the `user_id=` kwarg through the file pipeline (for billing/memory identity and signature compatibility), but the path helpers ignore `user_id` for disk layout so the bucket is keyed by `thread_id` alone:
...
- Files stored in thread-isolated directories keyed by `thread_id` alone (`threads/{thread_id}/user-data/uploads`; thread-only tenancy). Upload helpers accept a `user_id=` kwarg (threaded explicitly by IM channels, resolved from `get_effective_user_id()` for HTTP/embedded callers) but it is retained only for signature compatibility and does not change the disk bucket
