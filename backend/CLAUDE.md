...
1. **ThreadDataMiddleware** - Creates per-thread directories keyed by `thread_id` alone (`backend/.deer-flow/threads/{thread_id}/user-data/{workspace,uploads,outputs}`) and is the single source of truth for those paths (injected on `state["thread_data"]`; downstream path sites read them rather than re-resolving). Thread data is isolated by `thread_id`, never per-user, so multiple users sharing a thread share its materials (cfgpu-docs/thread-tenancy.md; `user_id` still scopes memory/agents/billing/tracing, just not thread-data disk). Web UI thread deletion follows LangGraph thread removal with Gateway cleanup of the local thread directory
...
14. **MemoryMiddleware** - Queues conversations for async memory update (filters to user + final AI responses); only loaded when `memory.enabled` (MLM-driven agents set `memory.enabled=false` + `mlm.enabled=true` and use MlmMiddleware instead)
...
19. **HumanApprovalMiddleware** - Pauses high-cost tool calls for human confirmation via `interrupt()` (optional; enabled when `ask=True` in runtime config AND `approval_required_tools` is set in agent config). On first pass: emits `tool_approval_required` SSE event and interrupts. Approval is **all-or-nothing per AIMessage batch**: the graph resumes only once every matched tool call in that response has a decision in `state.tool_approvals` (`decided_ids == pending_ids`). On resume: reads decisions from `state.tool_approvals` (merge reducer `merge_tool_approvals`, so decisions accumulate across multiple partial resumes); approved calls proceed with (possibly modified) args; rejected calls are retained in AIMessage.tool_calls (so every ToolMessage has a matching tool_call_id) and returned as error ToolMessages — routing still ends because `should_continue` checks `messages[-1]` which is the ToolMessage. On a **partial resume** (only some tools decided), the middleware re-emits the SSE + re-interrupts with **only the still-undecided subset** (already-decided tools are filtered out of the payload so the client doesn't re-confirm them); the final apply step still uses the full merged decision set. Supports fnmatch patterns for tool name matching (e.g. `cfdream_generate_*`). Must be before MessageStreamMiddleware and ClarificationMiddleware.
20. **MessageStreamMiddleware** - Emits semantic custom events at two hook points: `wrap_model_call` emits `ai_message` (AI reply text + tool_calls + usage from `usage_metadata`) after each main-agent LLM call; `wrap_tool_call` emits `tool_result` after each tool execution, where `content` is **always a JSON object** via `_structure_content` (JSON object passthrough; JSON array → `{"items": [...]}`; non-JSON text → `{"message": ...}`). The parse gate (`max_structured_bytes`, default 64 KiB) is **decoupled** from the text-truncation length (`max_content_chars`, default 4096): a large-but-legitimate JSON result (web_search's pretty-printed array, list_models) still reaches the client structured rather than degrading to a truncated string; only a pathological JSON beyond the hard cap (e.g. base64 embedded in the result, BUG-002 family) degrades to a truncated `{"message": ...}`, and non-JSON prose is always truncated at `max_content_chars`. For MCP tools, any `structuredContent` (carried on `ToolMessage.artifact["structured_content"]`, mapped there by langchain-mcp-adapters) is merged into the emitted event `content` — a client-only side channel the model never saw, so the client receives the full result (cfdream `generate_*` `usage`/`payload`, `understand_vision` `reasoning_content`/`usage`/`payload`) without it bloating (and being truncated out of) the lean tool result the LLM sees. `MaterialsMiddleware._rewrite_result` preserves this key alongside `items` so `generate_*` artifact events keep it too. **After emit, `MessageStreamMiddleware._strip_structured_side_channel` removes `structured_content` from the returned `ToolMessage`** (preserving sibling keys like `items`, collapsing an emptied artifact to `None`) so it does not persist in the checkpoint — it was never model context, the checkpoint has no reader for it, and its `payload` can carry presigned URLs (cfdream `reference_images`) we keep out of persisted state (materials I9/I10). This is the clean split: `content` → `ToolMessage` → LLM context; `structuredContent` → downstream event → client only. Two **emit-only** suppressions gate the downstream event without ever touching `ToolMessage.content` (the LLM always sees the full result): (a) **empty search** — a `progress` `web_search`/`image_search` whose structured content carries an `error` key (its `No results found`/`No images found` sentinel, matched by key presence not value) emits no `tool_result`, so the client gets no empty-result card while the LLM still sees the error and stops retrying (pairs with `loop_detection.tool_freq_overrides.{web_search,image_search}`); (b) **artifact tool with no items** — an `artifact` tool emits an `artifact` event only when it carries stable items, otherwise it suppresses an in-flight intermediate (`generate_*`/`task_*` polling a not-yet-ready URL, `status="success"`) or falls back to a `tool_result` on a terminal failure (`status="error"`) so the client still learns the generation failed; a failed-rehost degraded success (`status="success"`, unstable material → no items) is suppressed like any intermediate (I5: unstable is never delivered). The middleware does not synthesize a top-level `usage` field — tools that report token cost embed it inside their own result content. Pair with `stream_event_types=["custom"]` to receive a clean event stream without LangGraph's automatic `values`/`messages` noise. Naturally excludes SummarizationMiddleware internal calls, DynamicContextMiddleware/MLMMiddleware injections, and HumanApprovalMiddleware artificial ToolMessages (none pass through the wrap hooks). Always injected; no runtime switch.
21. **ClarificationMiddleware** - Intercepts `ask_clarification` tool calls, interrupts via `Command(goto=END)` (must be last)

**Other `build_middlewares` middlewares** (not in the numbered list above):
- **DynamicContextMiddleware** / **MlmMiddleware** — context injectors (current date + optional file-memory; multi-level DB memory). Injected once as a frozen first-turn `<system-reminder>`.
- **ArtifactUrlGuardMiddleware** — deterministic repair of stale / cross-object presigned URLs in tool-call args, via `wrap_tool_call`/`awrap_tool_call`. `present_urls` stores cfdream/OSS/TOS/S3 presigned URLs verbatim into `ThreadState.artifacts`; their signature (`?Expires=...&Signature=...`, `?X-Tos-...`, `?X-Amz-...`) is bound to a single object and 403s if reused on another path. After `SummarizationMiddleware` compresses tool results, a model rebuilding a download command (e.g. `curl`) from the lossy summary often keeps each object's correct *path* but pastes one object's signature onto several paths. The guard scans tool args for URLs, matches each by **object identity** (scheme+host+path, query stripped — provider-agnostic, never parses signature params) against the un-summarized `artifacts`, and rewrites matches to the authoritative full URL via `ToolCallRequest.override`. Unknown/external URLs and the no-artifacts case pass straight through. The only failure mode it cannot fix is a URL whose path was also corrupted (no identity left to match). Tests: `tests/test_artifact_url_guard_middleware.py`.
- **UninterruptibleToolMiddleware** / **RuntimeConfigMiddleware** — director-agent additions registered after HumanApproval (see `build_middlewares` for ordering rationale); shield non-cancellable cfdream tool calls from mid-flight hard cancel (BUG-009) and apply per-run client runtime config (config.skills / config.models clamp) respectively.
...
- Physical: `backend/.deer-flow/threads/{thread_id}/user-data/...` (thread-only tenancy; see cfgpu-docs/thread-tenancy.md), `deer-flow/skills/`
...
- Each ACP agent uses a per-thread workspace at `{base_dir}/threads/{thread_id}/acp-workspace/` (thread-only tenancy). The workspace is accessible to the lead agent via the virtual path `/mnt/acp-workspace/` (read-only). In docker sandbox mode, the directory is volume-mounted into the container at `/mnt/acp-workspace` (read-only); in local sandbox mode, path translation is handled by `tools.py`
