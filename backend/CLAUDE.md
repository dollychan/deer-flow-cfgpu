...
- **UninterruptibleToolMiddleware** / **RuntimeConfigMiddleware** — cf-dream-agent additions registered after HumanApproval (see `build_middlewares` for ordering rationale); shield non-cancellable cfdream tool calls from mid-flight hard cancel (BUG-009) and apply per-run client runtime config (config.skills / config.models clamp) respectively.
