...
def _resolve_attachments(thread_id: str, artifacts: list[str], *, user_id: str | None = None) -> list[ResolvedAttachment]:
    ...
    paths = get_paths()
    # Thread-only tenancy (thread-tenancy.md §4.1): artifacts resolve by thread_id alone.
    # The ``user_id`` param is kept for call-site signature compat but not used for disk layout.
