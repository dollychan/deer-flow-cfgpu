...
| 方向 | Topic | 说明 |
|------|-------|------|
| 上游 → 智能体服务 | `$AGENT_TASKS` | 所有上行消息：task（含 HIL resume）、cancel、ping |
| 智能体服务 → 上游 | `$AGENT_RESULTS` | 结果回调：progress / result / error / pong 消息 |

**单 Topic 设计**：所有上行消息（task、cancel、ping）统一发往 `$AGENT_TASKS`。Consumer poll-loop 不节流，全速拉取并立即 ACK，消息按 `thread_msg_seq` 顺序落入本地 `thread_msg_queue`；Semaphore 只控制任务执行出口（dispatch），不阻塞 poll。cancel 消息随 task 一起被立即拉取入库，不受 task 积压影响，响应延迟与两 topic 设计等同。`thread_msg_seq` 由客户端统一维护（task、cancel 均携带），Consumer 可精确确定 cancel 在会话中的位置，实现 cancel barrier 语义。

**Topic 类型**：两个 topic 均使用**普通消息**（非顺序消息）。同一 thread_id 的路由亲和性由 Consumer 服务内部的运行状态表保证，详见《Consumer 运行管理设计》。
...
| message_seq | int | 下行时 | 下行回复消息流的顺序号，从 1 开始递增，用于接收端排序和丢包检测（per message_id）。0 表示不适用（上行消息置 0） |
...
| thread_msg_seq | int | 是（上行） | 消息在 thread 内的顺序号，per-thread 从 1 开始递增，由发送方负责维护。用于多 Consumer 实例下恢复消息在会话中的准确顺序。ping 消息不携带 thread_id，此字段置 0 或省略 |
...
| 消息类型 | 前置条件 | 说明 |
|----------|----------|------|
| task | 无 | 发起新的 agent 执行轮次，或多轮对话的后续用户消息。当同 thread_id 有 task 正在执行时，新消息自动排队（followup）或按 `message_mode` 处理。`payload.command` 非空时为 HIL resume 消息 |
| cancel | 无 | 取消指定 thread 的当前执行及其之前尚未开始的排队 task；无运行中 task 时仅清理队列 |
| ping | 无 | 健康检查；Consumer Instance 回复 pong |

**Consumer 使用单一 SimpleConsumer 实例**消费 `$AGENT_TASKS`。poll-loop 不节流，全速拉取所有消息并立即 ACK；ping 在 poll-loop 内即时处理，task 和 cancel 按 `thread_msg_seq` 写入 `thread_msg_queue`；Semaphore 仅控制任务执行出口。
...
|------------------------|-----------|---------|
| `["custom"]` | ai_message、tool_result、tool_approval_required、safety_termination、warning、subagent task 事件等 custom 类型 | **推荐**，配合 MessageStreamMiddleware 使用 |
...
取消指定 thread 的当前执行，并将队列中位于本条 cancel 之前（`thread_msg_seq` 更小）尚未开始的 followup task 标记为 `prefix`——其 messages 将作为上下文前缀合并进 cancel 之后的下一轮 run，保证 LLM 能感知被取消消息的内容。cancel 之后的 task 正常执行。若 thread 当前无运行中 task，直接对积压的 followup 执行 prefix 转换。每条被标记 prefix 的 task 均会收到 `result(status="cancelled")` 通知。
...
**路由**：cancel 消息经由 `$AGENT_TASKS` poll-loop 写入 `thread_msg_queue`（与 task 消息共享同一有序队列），Consumer 通过 `thread_msg_seq` 确定 cancel 在会话中的精确位置，cancel_watcher 在检测到 cancel 行后取消当前 runner_task。
...
`instance_id` 可选。**任意 Consumer 实例均可处理 ping，消息无需路由到特定实例**。指定 `instance_id` 时，接收消息的 Consumer 查询共享的 `consumer_instances` 表，获取目标实例的状态和最近心跳时间，并将查询结果附在 pong 中回复；不指定时直接回复自身的 `instance_id`。Consumer 实例 ID 格式及注册机制见《Consumer 运行管理设计 — 实例管理》。
...
data 由 agent 自定义，通过 LangGraph `stream_writer` 显式 emit。`stream_event_types` 含 `"custom"`（推荐）时下行。目前有六类固定 schema：
...
| usage | 本次 LLM 调用的 token 用量，直接来自 `AIMessage.usage_metadata`（由 LangChain provider 自动填充）。**不含** subagent 子调用的 token（subagent 合并由 `TokenUsageMiddleware` 在 `after_model` 阶段处理，晚于本事件）。provider 不支持时缺省（字段不存在） |
| usage.input_tokens | 本次调用输入 token 数 |
| usage.output_tokens | 本次调用输出 token 数 |
| usage.total_tokens | input + output 合计 |
| usage.input_token_details | 输入 token 明细（如 cache_read、cache_creation），provider 不支持时为空对象或缺省 |
| usage.output_token_details | 输出 token 明细（如 reasoning），provider 不支持时为空对象或缺省 |
...
正常完成（cfgpu generate 类工具，含 usage）：
...
| usage | 工具调用的资源消耗，来自 `ToolMessage.additional_kwargs["usage"]`。**仅在工具自身上报 usage 时存在**（如 cfgpu generate 类工具调用外部模型 API）；普通工具无此字段。具体字段结构由 cfgpu MCP server 定义（待补充） |
...
**Safety 截断通知**（`SafetyFinishReasonMiddleware` 在 provider 因安全原因截断响应且带有未完整 tool_calls 时 emit）：
...
```json
{
  ...
  "data": {
    ...
    "detector": "openai_content_filter",
...
| 字段 | 说明 |
|------|------|
| type | 固定值 `"safety_termination"` |
| detector | 触发的检测器名称（如 `openai_content_filter`、`anthropic_refusal`、`gemini_safety`） |
| reason_field | provider 响应中触发截断的字段名（如 `finish_reason`、`stop_reason`） |
| reason_value | 该字段的具体值（如 `"content_filter"`、`"refusal"`、`"SAFETY"`） |
| suppressed_tool_call_count | 被取消的 tool_call 数量 |
| suppressed_tool_call_names | 被取消的工具名列表 |
| thread_id | 当前 thread ID（可选，从 runtime context 取，可能为 null） |

**消费者处理**：收到此事件时，应关闭所有因对应 `ai_message` 中 tool_calls 而建立的"等待 tool_result"占位——这些工具调用已被中止，不会产生 `tool_result` 事件。
...
每个 task 有且仅有一条 result 消息，在 agent 执行结束后推送。MQ ack 在消息 poll 后 dispatch 时立即完成，与执行结果无关。
...
```json
{
  ...
  "final_state": {
    "title": "AI发展趋势分析",
    "messages": [
      { "type": "human", "id": "...", "content": "用户输入" },
      { "type": "ai",    "id": "...", "content": "AI 回复..." }
    ],
    "artifacts": [...],
    "todos": [...],
    "uploaded_files": [...],
...
>
> `final_state` 是本次 run 结束后 LangGraph checkpoint 的**完整全量 state**（去除内部 `__pregel_*` / `__interrupt__` 键），不做增量计算。所有字段均反映 thread 当前最新值，客户端应直接替换本地对应字段。
>
> | 字段 | 类型 | 说明 |
> |------|------|------|
> | `messages` | array | **thread 全量对话历史**，含本次 run 之前的所有消息。客户端应以此替换本地 messages，而非追加 |
> | `title` | string \| null | thread 当前标题 |
> | `artifacts` | array | thread 全量 artifact 列表 |
> | `todos` | array \| null | 当前 TodoList（plan mode 场景） |
> | `uploaded_files` | array \| null | 本次 run 上传的文件列表 |
> | `sandbox` | object \| null | sandbox 运行时信息（内部字段，客户端通常无需使用） |
> | `thread_data` | object \| null | thread 级别元数据（内部字段，客户端通常无需使用） |
...
**指定 ping（有 `instance_id`）**：

```json
{
  "instance_id": "consumer-hostname-99999",
  "status": "active",
  "last_heartbeat": "2026-05-26T08:00:00.000Z",
  "host_instance_id": "consumer-hostname-12345"
}
```

| 字段 | 说明 |
|------|------|
| `instance_id` | 被查询的目标实例 ID（仅指定 ping 时出现） |
| `status` | 目标实例状态：`active`、`draining`、`not_found` |
| `last_heartbeat` | 目标实例最近心跳时间（ISO 8601 UTC）；`status=not_found` 时缺省 |
| `host_instance_id` | 实际**回复**该 ping 的 Consumer 实例 ID（仅指定 ping 时出现） |
...
timeout_seconds 在 task 的 `config` 中由客户端按需设置。Consumer 内部使用 asyncio timeout 控制后台执行时长，超时后发送 `error(AGENT_TIMEOUT)`。MQ ack 在消息 poll 后 dispatch 时已立即完成，与超时无关。
