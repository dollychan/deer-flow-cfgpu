...
`MQStreamBridge` 在每个 run 的上下文（`_RunContext`）中维护一个 `buffered_events` 列表，记录**实际已发送给上游**的 custom 事件数据：

```python
# MQStreamBridge.publish() — 发送成功后追加缓冲
if event == "custom" and isinstance(data, dict):
    ctx.buffered_events.append(data)
```

缓冲发生在所有过滤条件通过之后（`stream_events=False` 时整轮被抑制、`stream_event_types` 白名单不匹配时抑制），因此 `buffered_events` 的内容与实际下行给上游的 custom 事件**完全一致**，不多也不少。

`stream_events=false` 时所有 progress 事件均被抑制，`buffered_events` 自然为空，无需额外处理。

执行结束后，AgentRunner 通过 `bridge.get_buffered_events(run_id)` 取出缓冲列表的副本，存入 `result_cache`。

### 6.2 result_cache 结构

`result_cache` 由 `run_registry.mark_processed()` 持久化到 `processed_messages` 表，用于重复投递的幂等重放。

**stream_events=true（推荐模式）**：
...
```json
{
  ...
  "stream_events": true,
  "events": [
    { "type": "ai_message", "message_id": "msg_001", "content": "根据你的描述，我来帮你生成这张图片。", "tool_calls": [{ "id": "call_abc123", "name": "cfgpu__generate_image", "args": {} }] },
    { "type": "tool_result",  "message_id": "msg_002", "tool_call_id": "call_abc123", "name": "cfgpu__generate_image", "content": "{\"image_url\": \"...\"}", "status": "success" },
    { "type": "ai_message", "message_id": "msg_003", "content": "图片已生成，请查看。", "tool_calls": [] }
  ]
}
```

**stream_events=false（非流式模式）**：
...
```json
{
  ...
  "final_state": {
    "title": "AI 发展趋势分析",
    "messages": [{ "type": "human", "content": "用户输入" }, { "type": "ai", "content": "AI 回复" }],
    "artifacts": []
  }
}
```

**HIL 暂停（status=paused_for_approval）**：
...
```json
{
  ...
  "stream_events": true,
  "events": [
    { "type": "ai_message", "message_id": "msg_001", "content": "", "tool_calls": [{ "id": "call_abc123", "name": "cfgpu__generate_image", "args": {} }] }
  ],
  "tool_approval_required": {
    "type": "tool_approval_required",
    "tool_calls": [{ "id": "call_abc123", "name": "cfgpu__generate_image", "args": {} }]
  }
}
```
...
{
  "status": "failed",
  "error": { "code": "AGENT_TIMEOUT", "message": "Agent execution timed out after 300s", "retriable": true }
}
```

### 6.3 重复投递重放（replay）

当同一 `message_id` 重复到达时，`TaskConsumer` 检测到 `processed_messages` 已有记录，调用 `MQStreamBridge.replay()` 按原始下行顺序重发：

```
replay 发送顺序：
  1. result_cache["events"] 中的每条 custom 事件（封装为 progress 消息）
     ← stream_events=true 时存在；stream_events=false 时为空列表，跳过
  2. result_cache["tool_approval_required"]（若存在）
     ← 仅 HIL 暂停场景
  3. 终止信封：error（若 result_cache 含 "error" key）或 result
```

重放的消息流与首次执行的下行流完全一致（相同 `message_id`、相同事件内容、相同 `message_seq` 起始），客户端无需区分首次接收与重放。

### 6.4 result 消息中的 final_state

`stream_events=true` 时，result payload **不含** `final_state`（执行过程已通过 custom 事件全量实时下行，result 仅作终止信号）：

```json
{ "status": "success" }
```

`stream_events=false` 时，result payload **包含** `final_state`（客户端无法从 progress 获取任何内容，需在 result 中一次性接收）：

```json
{
  "status": "success",
  "final_state": { "title": "...", "messages": [...], "artifacts": [...] }
}
```
