...
- **If the client restricted the models** for this task, you'll see a manual-mode `<system-reminder>` listing the allowed image model IDs; choose only from that list.
- Once chosen, hold the model consistent across every image in the production unless the user changes it (part of the locked Production Spec).
