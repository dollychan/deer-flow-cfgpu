...
4. **`auto` when unconstrained.** `model="auto"` scores enabled models by quality/speed/cost; pass a **list** to constrain the pool. Always honor a **locked Production Spec** model over auto.
